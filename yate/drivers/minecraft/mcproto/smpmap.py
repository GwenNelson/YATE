...
from datautils import unpack_varint
...
class ChunkDataShort(ChunkData):
    ...
    def unpack(self, buff):
	b = Buffer(data=buff.buff)
        block_bits = b.unpack('B')
	uses_palette = False

	if block_bits <= 9:
           uses_palette = True
           palette_len = b.unpack_varint()
           palette      = []
           for i in xrange(palette_len): palette.append(b.unpack_varint())

	#data_longs = struct.unpack('>Q',buff.read(8))[0]
	data_longs = b.unpack_varint()
        block_data = []
        for i in xrange(data_longs):
            block_data.append(b.unpack_long())
